...
(function () {
  ...
  this.tmpl = function tmpl(str, data) {
    ...
    var fn = /^[-a-zA-Z0-9]+$/.test(str) ? cache[str] = cache[str] || tmpl(document.getElementById(str).innerHTML) : // Generate a reusable function that will serve as a template
...
Vvveb.Components = {
  ...
  matchNode: function matchNode(node) {
    ...
    if (node.attributes && node.attributes.length) {
      ...
      for (var i in node.attributes) {
        if (node.attributes[i]) {
          ...
          if (attr in this._attributesLookup) {
            component = this._attributesLookup[attr]; //currently we check that is not a component by looking at name attribute
      ...
      for (var i in node.attributes) {
        attr = node.attributes[i].name;
        value = node.attributes[i].value; //check for node classes
  ...
  render: function render(type) {
    ...
    var propertiesElement = $(propertiesElement + " .tab-pane").each(function () {
    ...
    var fn = function fn(component, property) {
...
Vvveb.Builder = {
  ...
  // 计算左侧模块容器高度
  setBlocksContainerHeight: function setBlocksContainerHeight() {
    var navHeight = 0;
    $(".drag-elements .nav-tabs").each(function (i, node) {
  ...
  loadBlockGroups: function loadBlockGroups() {
  ...
  loadUrl: function loadUrl(url, callback) {
    var self = this;
    $("#select-box").hide();
    self.initCallback = callback;
      ...
      $(window.FrameWindow).on("scroll resize", function (event) {
        if (self.selectedEl) {
          ...
          $("#select-box").css({
            ...
            "height": self.selectedEl.outerHeight() //"display": "block"
        ...
        if (self.highlightEl) {
          ...
          highlightBox.css({
            ...
            "height": self.highlightEl.outerHeight() //"display": "block"
  ...
  _frameLoaded: function _frameLoaded() {
    ...
    self.frameHead = $(window.FrameDocument).find("head"); //insert editor helpers like non editable areas
  ...
  loadNodeComponent: function loadNodeComponent(node) {
    ...
    var componentType;
    if (data) componentType = data.type;else componentType = Vvveb.defaultComponent; // Vvveb.Components.render(component);
          ...
          try {
            if (event.originalEvent) {
              if (offset.top < y - halfHeight || offset.left < x - halfWidth) {
                if (isIE11) self.highlightEl.append(self.dragElement);else self.dragElement.appendTo(parent);
              } else {
                if (isIE11) self.highlightEl.prepend(self.dragElement);else self.dragElement.prependTo(parent);
              }
  ...
  _initBox: function _initBox() {
    ...
    $("#add-section-btn").on("click", function (event) {
      ...
      var outerHeight = $(window.FrameWindow).height() + self.frameDoc.scrollTop(); //check if box is out of viewport and move inside
  ...
  _initDragdrop: function _initDragdrop() {
    ...
    $('.drag-elements-sidepane ul > li > ol > li').on("mousedown touchstart", function (event) {
      $this = $(this);
      $("#component-clone").remove();
      if ($this.data("drag-type") == "component") self.component = Vvveb.Components.get($this.data("type"));else self.component = Vvveb.Blocks.get($this.data("type"));
    ...
    $('body').on('mousemove touchmove', function (event) {
      if (self.iconDrag && self.isDragging == true) {
        ...
        elementMouseIsOver = document.elementFromPoint(x - 60, y - 40); //if drag elements hovers over iframe switch to iframe mouseover handler	
  ...
  removeHelpers: function removeHelpers(html) {
    var keepHelperAttributes = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    //tags like stylesheets or scripts 
    html = html.replace(/<.*?data-vvveb-helpers.*?>/gi, ""); //attributes
  ...
  getHtml: function getHtml() {
    var keepHelperAttributes = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : true;
  ...
  setHtml: function setHtml(html) {
    ...
    if (this.runJsOnSetHtml) self.frameBody.html(body);else window.FrameDocument.body.innerHTML = body; //below methods brake document relative css and js paths
    //return self.iframe.outerHTML = html;
    //return self.documentFrame.html(html);
    //return self.documentFrame.attr("srcdoc", html);
  },
  saveAjax: function saveAjax(fileName, startTemplateUrl, callback, saveUrl) {
    var data = {};
    ...
    $.ajax({
      type: "POST",
      url: saveUrl,
      //set your server side save script url
      data: data,
      cache: false,
  ...
  setDesignerMode: function setDesignerMode() {
    var designerMode = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
...
Vvveb.CodeEditor = {
  ...
  destroy: function destroy(element) {//this.isActive = false;
  },
...
Vvveb.Gui = {
  ...
  blockSearch: function blockSearch() {
    searchText = this.value;
  ...
  clearBlockSearch: function clearBlockSearch() {
    $(".block-search").val("").keyup();
  },
  addBoxComponentSearch: function addBoxComponentSearch() {
    searchText = this.value;
  ...
  addBoxBlockSearch: function addBoxBlockSearch() {
    searchText = this.value;
  ...
  //layout
  togglePanel: function togglePanel(panel, cssVar) {
    var panel = $(panel);
    var body = $("body");
  ...
  toggleFileManager: function toggleFileManager() {
    Vvveb.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
  },
  toggleLeftColumn: function toggleLeftColumn() {
    Vvveb.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
  },
  toggleRightColumn: function toggleRightColumn() {
    var rightColumnEnabled = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;
...
Vvveb.StyleManager = {
  setStyle: function setStyle(element, styleProp, value) {
    return element.css(styleProp, value);
  },
  _getCssStyle: function _getCssStyle(element, styleProp) {
    var value = "";
  ...
  getStyle: function getStyle(element, styleProp) {
    return this._getCssStyle(element, styleProp);
  }
};
Vvveb.ContentManager = {
  getAttr: function getAttr(element, attrName) {
    return element.attr(attrName);
  },
  setAttr: function setAttr(element, attrName, value) {
...
Vvveb.Sections = {
  ...
  getSections: function getSections() {
    var sections = [];
    var sectionList = $('> section, > header, > footer', window.FrameDocument.body);
  ...
  init: function init() {
    $(this.selector).on("click", "> div", function (e) {
      ...
      var node = window.FrameDocument.querySelector("#".concat(uuid));
      var $node = $(node);
...
Vvveb.FileManager = {
  ...
  allowedComponents: {},
  init: function init() {
    var allowedComponents = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : {};
    ...
    $(this.tree).on("click", "li[data-component] label ", function (e) {
      ...
      node.click(); //Vvveb.Builder.selectNode(node);
      //Vvveb.Builder.loadNodeComponent(node);
      //e.preventDefault();
      //return false;
  ...
  addPage: function addPage(name, data) {
  ...
  addComponent: function addComponent(name, url, title, page) {
    $("[data-page='" + page + "'] > ol", this.tree).append(tmpl("vvveb-filemanager-component", {
      name: name,
      url: url,
      title: title
    }));
    ...
    function getNodeTree(node, parent) {
      if (node.hasChildNodes()) {
        for (var j = 0; j < node.childNodes.length; j++) {
          ...
          if (child && child["attributes"] != undefined && (matchChild = Vvveb.Components.matchNode(child))) {
            if (Array.isArray(allowedComponents) && allowedComponents.indexOf(matchChild.type) == -1) continue;
            element = {
  ...
  getCurrentFileName: function getCurrentFileName() {
    if (this.currentPage) {
  ...
  reloadCurrentPage: function reloadCurrentPage() {
    if (this.currentPage) return this.loadPage(this.currentPage);
  },
  loadPage: function loadPage(name) {
    var allowedComponents = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    var disableCache = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;
...
function launchFullScreen(document) {
  if (document.documentElement.requestFullScreen) {
    if (document.FullScreenElement) document.exitFullScreen();else document.documentElement.requestFullScreen(); //mozilla		
  } else if (document.documentElement.mozRequestFullScreen) {
    if (document.mozFullScreenElement) document.mozCancelFullScreen();else document.documentElement.mozRequestFullScreen(); //webkit	  
  } else if (document.documentElement.webkitRequestFullScreen) {
    if (document.webkitFullscreenElement) document.webkitExitFullscreen();else document.documentElement.webkitRequestFullScreen(); //ie	  
