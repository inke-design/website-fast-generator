...
(function ($) {
  $.fn.autocomplete = function (settings) {
    ...
    {
      var textInput = $(this);
      textInput.attr("name", textInput.attr("name") + "_text"); //create a new hidden input that will be used for holding the return value when posting the form, then swap names with the original input

      var hiddenInput = $('<input type=hidden name="' + textInput.attr("name") + '"/>');
      hiddenInput.val(textInput.val());
      textInput.after(hiddenInput);
      var valueInput = $(this).next(); //create the ul that will hold the text and values
        ...
        if (text != oldText && settings.minChars != null && text.length >= settings.minChars) {
          ...
          $.getJSON(settings.url, settings.parameters, function (data) {
            ...
            if (data) {
              ...
              list.css({
                /*top: textInput.offset().top + textInput.outerHeight(), left: textInput.offset().left,*/
                width: Math.max(100, textInput.outerWidth())
              }).html(items); //on mouse hover over elements set selected class and on click set the selected value and close list
      ...
      textInput.keydown(function (e) {
        ...
        if (e.which == 27) //escape
          ...
          {
            clear(); //invalidate previous selection
  ...
  $.autocompleteList = function (el, settings) {
    ...
    function setValue(value) {
      if (value == "" || value == undefined) return false; //			value = decodeURIComponent(value);
  ...
  $.tagsInput = function (el, settings) {
    ...
    function setList() {
      var values = {}; //console.log($('input[name="list[]"]', list).serialize());
